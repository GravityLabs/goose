...
public class GeeseContentExtractor {
   ...
   * @param config
   */
  public GeeseContentExtractor(Configuration config) {
    this.config = config;
  }


  public Article extractContent(String html){
        return performExtraction(html);
  }


  public Article performExtraction(String html){
        
        ParseWrapperGeese parseWrapper = new ParseWrapperGeese();
        Article article = null;
        try {

          Document doc = parseWrapper.parse(html);
          ...
          if (article.getTopNode() != null) {
            ...
            //if (config.isEnableImageFetching()) {
              ...
              //imageExtractor = getImageExtractor(httpClient);
